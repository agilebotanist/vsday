...
| **FR-22** | VSDay SHALL offer two scaling strategies, selectable with `vsday.scaleStrategy` and switchable by a single command that re-applies the current step. `uniform` (**the default**) SHALL deliver the whole scale through `window.zoomLevel` — at a per-step amount derived from `vsday.fontScale.ratio` — and leave font settings at their baseline, so that every pane scales identically, including those with no font-size setting (Explorer, Extensions view, Settings UI, tab labels, extension webviews). `textFirst` SHALL deliver the scale through the font settings with a small zoom nudge. The reported scale percentage SHALL mean the same thing under both. | U `fontScale`, `fontScaleController`; I `uniformScale`, `fontScale`             |
| **FR-23** | On activation VSDay SHALL reconcile the settings with the recorded step and strategy, writing only where they differ, so that an upgrade, a hand-edited size, or a strategy changed directly in `settings.json` cannot leave a half-applied state. It SHALL NOT write in response to a strategy value being **cleared**, since that is what a bulk settings reset looks like part-way through.                                                                                                                                                                                                                                                                           | U `fontScaleController`; I `uniformScale`                                       |
...
| FR-22               | `zoomPerStepForRatio` in `src/fontScale.ts`; `applyStep`/`applyZoom`       |
| FR-23               | `FontScaleController.reconcile`; the activation hook in `src/extension.ts` |
